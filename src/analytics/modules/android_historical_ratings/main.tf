locals {
  table_name = "${terraform.workspace}_analytics_android_historical_ratings"
}

module "android_historical_ratings" {
  source                   = "../../libraries/analytics_s3"
  name                     = "analytics-android-historical-ratings"
  service                  = var.service
...
resource "aws_glue_catalog_table" "this" {
  ...
  storage_descriptor {
    location      = "s3://${module.android_historical_ratings.bucket_name}/"
