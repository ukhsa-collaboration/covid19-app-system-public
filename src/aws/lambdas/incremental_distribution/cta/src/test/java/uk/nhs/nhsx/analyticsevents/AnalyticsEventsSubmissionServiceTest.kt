...
class AnalyticsEventsSubmissionServiceTest {
    ...
    private fun createService(bucketName: BucketName, firehoseEnabled: Boolean = false) =
        AnalyticsEventsSubmissionService(
            awsS3 = fakeS3,
            objectKeyNameProvider = { ObjectKey.of("key") },
            bucketName = bucketName,
            events = events,
            firehoseClient = FirehoseClient(firehoseEnabled, "streamName", kinesisFirehose, retrier),
        )
    ...
    fun `retry on temporary firehose failure`() {
        val captures = mutableListOf<PutRecordRequest>()
        val job = FlakyJob(result = mockk<PutRecordResult>(), error = ServiceUnavailableException("foo"), nErrors = 1)
...
class RetryTest {
    ...
    private val retrier: Retrier = Retrier()

    @Test
    fun `retry returns first success`() {
        val expected = "result"
        val job = FlakyJob(expected, nErrors = 0)
        val actual = retrier.retry(times = 2, delayMillis = 0, cause = Exception::class, block = job::invoke)
    ...
    fun `retry on failure`() {
        val expected = "result"
        val job = FlakyJob(expected, nErrors = 1)
        val actual = retrier.retry(times = 2, delayMillis = 0, cause = Exception::class, block = job::invoke)
        expectThat(expected).isEqualTo(actual)
    }
    ...
    fun `retry throws other exceptions`() {
        val job = FlakyJob("result", nErrors = 3, error = ServiceUnavailableException("foo"))
        expectCatching {
            retrier.retry(times = 2, delayMillis = 0, cause = IllegalStateException::class, block = job::invoke)
    ...
    fun `retry throws broad exceptions`() {
        val job = FlakyJob("result", nErrors = 3, error = Exception("foo"))
        expectCatching {
            retrier.retry(times = 2, delayMillis = 0, cause = IllegalStateException::class, block = job::invoke)
    ...
    fun `retry stops after n retries`() {
        val job = FlakyJob("result", nErrors = 3, error = ServiceUnavailableException("foo"))
        expectCatching {
            retrier.retry(times = 2, delayMillis = 0, cause = ServiceUnavailableException::class, block = job::invoke)
        }.isFailure().isA<ServiceUnavailableException>()
