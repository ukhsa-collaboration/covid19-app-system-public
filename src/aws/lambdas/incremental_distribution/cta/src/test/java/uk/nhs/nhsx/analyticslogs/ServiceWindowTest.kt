...
class ServiceWindowTest {
    ...
    fun `return correct window when time is 1am`() {
        val instantSetTo1am = Instant.from(ZonedDateTime.of(2021, 1, 2, 1, 0, 0, 0, UTC))
        val expectedStartTime = Instant.from(ZonedDateTime.of(2021, 1, 1, 0, 0, 0, 0, UTC))
        val expectedEndTime = Instant.from(ZonedDateTime.of(2021, 1, 1, 23, 59, 59, 0, UTC))
    ...
    fun `return correct window when time is 6am`() {
        val instantSetTo6am = Instant.from(ZonedDateTime.of(2021, 1, 2, 6, 0, 0, 0, UTC))
        val expectedStartTime = Instant.from(ZonedDateTime.of(2021, 1, 1, 0, 0, 0, 0, UTC))
        val expectedEndTime = Instant.from(ZonedDateTime.of(2021, 1, 1, 23, 59, 59, 0, UTC))
