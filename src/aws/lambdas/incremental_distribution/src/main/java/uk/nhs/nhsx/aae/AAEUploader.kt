...
class AAEUploader(private val config: AAEUploadConfig, secretManager: SecretManager, private val events: Events) {
    ...
    fun uploadToAAE(s3Object: S3Object) {
        val uploadRequest = HttpRequest.newBuilder()
            .header(SUBSCRIPTION_HEADER_NAME, subscription)
            .header("Content-Type", s3Object.objectMetadata.contentType)
    ...
    companion object {
        ...
        fun getContent(obj: S3Object): ByteArray {
            obj.objectContent.use { `in` -> return `in`.readAllBytes() }
        }

        fun getFilename(obj: S3Object): String =
