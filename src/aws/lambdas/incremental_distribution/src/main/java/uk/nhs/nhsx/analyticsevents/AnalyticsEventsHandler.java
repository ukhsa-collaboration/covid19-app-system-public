...
public class AnalyticsEventsHandler extends RoutingHandler {
    ...
    public AnalyticsEventsHandler(Environment environment, Supplier<Instant> clock) {
        this(environment, awsAuthentication(ApiName.Mobile), signResponseWithKeyGivenInSsm(environment, clock), new AwsS3Client(), new PartitionedObjectKeyNameProvider(clock, UniqueId.ID));
    }

    public AnalyticsEventsHandler(Environment environment, Authenticator authenticator, ResponseSigner signer, S3Storage s3Storage, ObjectKeyNameProvider objectKeyNameProvider) {
