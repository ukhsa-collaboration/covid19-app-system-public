...
public class Handler extends RoutingHandler {
    ...
    public Handler() {
        this(
            ...
            datedSigner(SystemClock.CLOCK, Environment.fromSystem()),
            new AwsS3Client(),
    ...
    public Handler(Environment environment,
        ...
        handler = withoutSignedResponses(
            ...
            authenticator,
            routes(
                path(Method.POST, "/upload/high-risk-postal-districts", (r) -> service.upload(r.getBody())),
                path(Routing.Method.POST, "/upload/high-risk-postal-districts/health", (r) -> HttpResponses.ok())
            )
        );
    }
