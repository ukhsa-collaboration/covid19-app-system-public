...
class AnalyticsEventsSubmissionServiceTest {
    ...
    fun `accept payload with no mapped postal district is stored in s3`() {
        val fakeS3 = FakeS3()
        val service = AnalyticsEventsSubmissionService(fakeS3, { ObjectKey.of("key") }, BucketName.of("bucket"))
        service.accept(
            mapOf(
                "metadata" to mapOf(
        ...
        val storedObject: Map<String, Any> = Jackson.readJson(fakeS3.bytes.openStream(), Map::class.java) as Map<String, Any>
