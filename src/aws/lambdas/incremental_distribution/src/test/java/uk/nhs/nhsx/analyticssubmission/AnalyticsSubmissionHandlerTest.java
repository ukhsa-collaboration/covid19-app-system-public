...
public class AnalyticsSubmissionHandlerTest {
    ...
    public static String iOSPayloadFromNewMetrics(String startDate, String endDate) {
        String metrics = "    ,\"receivedVoidTestResultEnteredManually\" : 1," +
    ...
    public static String iOSPayloadFromWithMetrics(String startDate, String endDate, String postDistrict, String metrics) {
        return "{" +
                "  \"metadata\" : {" +
    ...
    private final AmazonKinesisFirehose kinesisFirehose = mock(AmazonKinesisFirehose.class);
    ...
    @Test
    public void acceptsiOSPayloadWithNewMetricFieldsAndReturns200() throws IOException {
        APIGatewayProxyRequestEvent requestEvent = ProxyRequestBuilder.request()
                .withMethod(HttpMethod.POST)
                .withPath("/submission/mobile-analytics")
                .withBearerToken("anything")
        ...
        assertThat(s3Storage.name, equalTo(objectKey.append(".json")));
        assertThat(s3Storage.bucket, equalTo(BUCKET_NAME));
        assertThat(new String(s3Storage.bytes.read(), StandardCharsets.UTF_8), equalTo(TestData.STORED_ANALYTICS_PAYLOAD_IOS_NEW_METRICS));
    }
    ...
    public void iosPayloadWithPostcodeNotFoundInMappingSavesPostDistrictAsUnknown() throws IOException {
        ...
        assertThat(s3Storage.name, equalTo(objectKey.append(".json")));
        assertThat(s3Storage.bucket, equalTo(BUCKET_NAME));
        assertThat(new String(s3Storage.bytes.read(), StandardCharsets.UTF_8), equalTo(TestData.STORED_ANALYTICS_UNKNOWN_POSTCODE_PAYLOAD_IOS));
    }
