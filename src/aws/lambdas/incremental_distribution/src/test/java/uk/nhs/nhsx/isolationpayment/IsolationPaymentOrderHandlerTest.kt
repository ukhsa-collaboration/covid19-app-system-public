...
class IsolationPaymentOrderHandlerTest {
    ...
    private val handler = IsolationPaymentOrderHandler(environment, authenticator, signer, service)
    ...
    fun `token create returns 201 when tokens is created`() {
        ...
        val tokenGenerationResponse =
            deserializeMaybe(response.body, TokenGenerationResponse::class.java)
                .orElseThrow()
    ...
    fun `token create returns 503 when feature is disabled`() {
        val handler = IsolationPaymentOrderHandler(environmentTokenCreationDisabled, authenticator, signer, service)
    ...
    fun `token update returns 200 when tokens is updated`() {
        ...
        val tokenUpdateResponse =
            deserializeMaybe(response.body, TokenUpdateResponse::class.java)
                .orElseThrow()

        assertThat(tokenUpdateResponse.websiteUrlWithQuery)
            .isEqualTo("https://test?ipcToken=some-id")
    }
