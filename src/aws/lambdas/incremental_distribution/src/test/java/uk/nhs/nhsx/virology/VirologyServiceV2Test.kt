...
class VirologyServiceV2Test {
    ...
    private val randomCountry = Country.of("random-country")
    ...
    fun `virology lookup with result available`() {
        ...
        val request = VirologyLookupRequestV2(pollingToken, Country.of("England"))
        val lookupResult = service.virologyLookupForV2(request)
    ...
    fun `exchanges cta token`() {
        ...
        val result = service.exchangeCtaTokenForV2(request) as CtaExchangeResult.AvailableV2
    ...
    @Test
    fun `exchanges cta token - returning not supported diagnosis keys submission`() {
        val ctaToken = CtaToken.of("cc8f0b6z")
        val testResultPollingToken = TestResultPollingToken.of("poll-token")
        val testOrder = TestOrder(ctaToken.value, testResultPollingToken.value, "sub-token")
        val testResult = TestData.positiveTestResultFor(testOrder.testResultPollingToken)
        ...
        val service = VirologyService(persistenceService, TokensGenerator(), clock)
        val request = CtaExchangeRequestV2(ctaToken, randomCountry)
        val result = service.exchangeCtaTokenForV2(request) as CtaExchangeResult.AvailableV2
    ...
    fun `exchanges cta token with pending test result`() {
        ...
        val service = VirologyService(persistenceService, TokensGenerator(), clock)
        val result = service.exchangeCtaTokenForV2(CtaExchangeRequestV2(ctaToken, country))
    ...
    fun `when download counter is 2, return not found`() {
        ...
        assertThat(service.exchangeCtaTokenForV2(CtaExchangeRequestV2(ctaToken, country))).isInstanceOf(CtaExchangeResult.NotFound::class.java)
