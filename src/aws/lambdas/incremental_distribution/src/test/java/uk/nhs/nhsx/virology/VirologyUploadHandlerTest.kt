...
class VirologyUploadHandlerTest {
    ...
    fun `accepts test result returns 202`() {
        ...
        val handler = VirologyUploadHandler(environment, Authenticator { true }, service)
    ...
    fun `invalid path returns 404`() {
        ...
        val handler = VirologyUploadHandler(environment, Authenticator { true }, uploadService)
        val requestEvent = ProxyRequestBuilder.request()
            .withMethod(HttpMethod.POST)
