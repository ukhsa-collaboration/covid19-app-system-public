  ...
  namespace :gen do
    NHSx::TargetEnvironment::CTA_TARGET_ENVIRONMENTS.each do |account, tgt_envs|
      tgt_envs.each do |tgt_env|
        ...
        desc "Generate virology tokens from a CSV file with headers: test_kit, test_result, test_end_date, number_of_tokens"
        task :"batch:#{tgt_env}" => [:"login:#{account}"] do
          include NHSx::Virology
          include NHSx::Generate
          config_file = JSON.parse(File.read(generate_test_config(tgt_env, account, $configuration)))
          generate_batch(config_file, $configuration)
        end
